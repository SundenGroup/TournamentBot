...
const FORMAT_INFO = {
  ...
  battle_royale: {
    name: 'Battle Royale',
    description: 'Shared lobbies, points per game (placement + kills). Single lobby, or groups advancing to a Grand Finals.',
  },
};
  ...
  embed.addFields(
    ...
    },
    {
      ...
      value: [
        ...
        '`create-rooms` — Retry creating missing match/lobby rooms',
        '`bracket` — View bracket/standings',
        '`seed set|list|randomize|clear` — Manage seeding',
        '',
        '**Battle Royale:** report by tapping the 🎮 Game buttons on each lobby\'s standings board — top 3–5 in finish order is enough. Tap a reported game to correct it. `/match games` lists pending games.',
      ].join('\n'),
      inline: false,
    },
    {
    ...
    },
    {
      ...
      value: [
        ...
        '`trial` — Start 7-day Pro trial',
        '`api-key` — Manage REST API key *(Studio)*',
        '`webhook` — Configure webhooks *(Studio)*',
        '`branding` — White-label branding *(Studio)*',
      ].join('\n'),
      inline: false,
    }
